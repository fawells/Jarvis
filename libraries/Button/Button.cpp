...
*/
Button::Button(uint8_t buttonPin, uint8_t buttonMode, unsigned int debounce)
{
  pin = buttonPin;
  pinMode(pin, INPUT);
  ...
  numberOfPresses = 0;
  numberOfPressesInEvent = 0;
  triggeredHoldEvent = true;
 
  debounceDelay = (debounce > 0) ? debounce : 0; 
  setActiveFader(FADER_OFF);
...
/*
|| @description
|| | Set mode to off/active (active incresing value, or active decreasing value)
|| #
*/
void Button::setActiveFader(uint8_t faderState)
{
    activeFader = faderState;
}
...
bool Button::isPressed(void)
{
  ...
  if (bitRead(state, CURRENT) != bitRead(state, PREVIOUS))
  {
    ...
    if (bitRead(state, CURRENT) == true)
    {
      ...
      if (millis() - releasedTime > debounceDelay)
      {        
         numberOfPresses++;
         numberOfPressesInEvent++;
         ...
         if (cb_onTriple)
         {
            if (millis() - tripleStartTime > tripleEventThreshold){
               numberOfPressesInEvent = 1;
               tripleStartTime = millis();
            }
         }
    ...
    else //the state changed to RELEASED
    {
      ...
      if (cb_onRelease && pressedStartTime != -1)
      {
        cb_onRelease(*this);  //fire the onRelease event
        
      }

      if (cb_onClick && !triggeredHoldEvent && pressedStartTime != -1)  //don't fire click if held
      {
      ...
      if (cb_onTriple)
      {
         if (numberOfPressesInEvent >= 3){
            numberOfPressesInEvent = 0;
            cb_onTriple(*this);  //fire the onTripleClick event AFTER the onRelease
         }
      }
...
/*
|| @description
|| | Set the triple event time threshold
|| #
*/
void Button::setTripleThreshold(unsigned int tripleTime)
{
  tripleEventThreshold = tripleTime;
}

/*
|| @description
|| | Set debounce delay time
|| #
*/
void Button::setDebounceDelay(unsigned int debounce)
{
...
/*
|| @description
|| | Register a handler for when this button is triple clicked
|| #
||
|| @parameter handler The function to call when this button is triple clicked
*/
void Button::tripleClickHandler(buttonEventHandler handler, unsigned int tripleTime /*=0*/)
{
  if (tripleTime > 0)
  {
    setTripleThreshold(tripleTime);
  }
  cb_onTriple = handler;
}
